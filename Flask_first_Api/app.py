...
def create_app(db_url=None):
    ...
    def add_claims_to_jwt(identity):
        # Lock in the data base se whether the user is admin are not dont go with out seeing admin id
        if identity == 1:
    ...
    def invalid_token_callback(jwt_header, jwt_payload):
        return(
            jsonify({"message":"Signature verification faild.", "error":"invaid_token"}), 401
        )
